...
const WS_URL_STORAGE_KEY = 'https://upgraded-octo-telegram-no5m.onrender.com';
...
function normalizeWsUrl(rawValue) {
  ...
  // Remove wrapping quotes if copied from code/editor.
  value = value.replace(/^['"]+|['"]+$/g, '');

  // Accept http(s) and map to ws(s).
  if (/^https?:\/\//i.test(value)) {
    value = value.replace(/^http/i, 'ws');
  }

  // Accept host-only values like: your-service.onrender.com
  if (!/^[a-z]+:\/\//i.test(value)) {
    value = `wss://${value}`;
  }
...
function readConfiguredWsUrl() {
  const queryUrl = new URLSearchParams(window.location.search).get('ws');
  if (queryUrl) {
    const normalizedQuery = normalizeWsUrl(queryUrl);
    if (normalizedQuery) {
      localStorage.setItem(WS_URL_STORAGE_KEY, normalizedQuery);
      return normalizedQuery;
    }
  }

  const storedUrl = normalizeWsUrl(localStorage.getItem(WS_URL_STORAGE_KEY) || '');
  if (storedUrl) {
    localStorage.setItem(WS_URL_STORAGE_KEY, storedUrl);
    return storedUrl;
  }

  return normalizeWsUrl(WS_URL);
}
...
function resolveWsUrl() {
  ...
  if (configuredWsUrl) {
    if (isValidWsUrl(configuredWsUrl)) {
      return configuredWsUrl;
    }
    return '__INVALID_WS_URL__';
  }
...
function connect() {
  ...
  if (!targetWsUrl) {
    showWarning(
      'Для GitHub Pages укажите WS_URL в docs/app.js или добавьте ?ws=wss://<ваш-render-сервис>.onrender.com',
      { persist: true }
    );
    return;
  }

  if (targetWsUrl === '__INVALID_WS_URL__') {
    showWarning('Неверный WS_URL. Допустимо: ws://..., wss://..., https://... или host без протокола.', { persist: true });
    return;
  }
